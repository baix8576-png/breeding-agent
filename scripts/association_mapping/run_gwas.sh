...
FORCE="false"
RUN_LOG=""
PIPELINE_NAME="association_mapping_gwas"

usage() {
  cat <<'EOF'
Usage: run_gwas.sh [options]
...
Options:
  ...
  --analysis-targets CSV    Analysis targets (default: gwas)
  ...
  --tmp-dir PATH            Temporary directory for tool spill files (default: workdir/tmp/association_mapping_gwas)
...
log() {
  printf '[association_mapping_gwas] %s\n' "$*"
}

warn() {
  printf '[association_mapping_gwas][warn] %s\n' "$*" >&2
}

fail() {
  printf '[association_mapping_gwas][error] %s\n' "$*" >&2
  exit 1
}
...
validate_posix_path() {
  ...
  local value="$2"
  if [[ "$value" == *\\* ]]; then
    fail "$label uses Windows-style backslashes. Use POSIX '/' paths for remote execution: $value"
  fi
}
  ...
  write_json_file "$audit_path" <<EOF
{
  ...
  "notes": [
    ...
    "Raw genotype, phenotype, and covariate inputs are read-only from this wrapper.",
    "GWAS interpretation requires population-structure and multiple-testing review."
  ]
}
EOF
}
...
normalize_threads
resolve_resource_limits
for path_value in "$WORKDIR" "$INPUT_ROOT" "$VCF_PATH" "$PLINK_PREFIX" "$PHENOTYPE_PATH" "$COVARIATE_PATH" "$TMP_DIR" "$LOG_DIR"; do
  if [[ -n "$path_value" ]]; then
    validate_posix_path "path" "$path_value"
  fi
done

OUT_DIR="$WORKDIR/results/association/gwas"
...
if [[ -z "$PLINK_PREFIX" && -n "$VCF_PATH" ]] && command_exists plink2; then
  TMP_PREFIX="$OUT_DIR/gwas_tmp_dataset"
...
if [[ -z "$PHENOTYPE_PATH" || ! -f "$PHENOTYPE_PATH" ]]; then
  fail "Phenotype table is required for GWAS."
fi
if ! command_exists plink2; then
  fail "plink2 is required for GWAS association mapping."
fi

escaped_request="$(json_escape "$REQUEST_TEXT")"
escaped_targets="$(json_escape "$ANALYSIS_TARGETS")"
cat > "$OUT_DIR/cohort_alignment.json" <<EOF
{
  "pipeline": "association_mapping_gwas",
...
if has_target "gwas"; then
  ...
  gwas_cmd=(plink2 --bfile "$PLINK_PREFIX" --threads "$THREADS" --memory "$MEMORY_MB" --pheno "$PHENOTYPE_PATH" --glm hide-covar --allow-no-sex --out "$OUT_DIR/gwas")
  if [[ -n "$COVARIATE_PATH" && -f "$COVARIATE_PATH" ]]; then
    gwas_cmd+=(--covar "$COVARIATE_PATH")
  fi
  "${gwas_cmd[@]}"
  gwas_result_file="$(find "$OUT_DIR" -maxdepth 1 -type f -name "gwas*.glm.*" -print | head -n 1)"
  executed_steps+=("plink2_glm")
fi

if [[ "${#executed_steps[@]}" -eq 0 ]]; then
  fail "No GWAS step executed. Ensure requested targets are valid."
fi
...

{
  ...
  echo
  if [[ -n "$gwas_result_file" && -f "$gwas_result_file" ]]; then
    for gwas_file in "$OUT_DIR"/gwas*.glm.*; do
      if [[ -f "$gwas_file" ]]; then
        echo "- $gwas_file"
      fi
    done
  ...
  echo "- results/association/gwas/cohort_alignment.json"
  echo "- results/association/gwas/model_spec.json"
  echo "- results/association/gwas/metrics.tsv"
} > "$OUT_DIR/README.md"

{
  echo "# GWAS Association Mapping Summary"
  echo
  echo "## Executed Steps"
  ...
  echo "- results/association/gwas/cohort_alignment.json"
  echo "- results/association/gwas/model_spec.json"
  echo "- results/association/gwas/README.md"
  echo "- results/association/gwas/metrics.tsv"
} > "$REPORT_DIR/gwas_summary.md"

log "GWAS association mapping completed successfully."
