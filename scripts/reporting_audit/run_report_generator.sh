...
THREADS="${GENEAGENT_REPORT_THREADS:-${GENEAGENT_REMOTE_CPU_CAP:-1}}"
...
Options:
  ...
  --pipeline NAME         Blueprint name (qc_pipeline/pca_pipeline/grm_builder/association_mapping_gwas/genomic_prediction)
  ...
  --dry-run               Write run manifest and planned subcommands without executing them
...
write_run_manifest() {
  local mode="$1"
  local manifest_path="$WORKDIR/results/report_generator_run_manifest.json"
  write_json_file "$manifest_path" <<EOF
{
  ...
  "outputs": {
    ...
    "figures": "$(json_escape "$FIGURE_OUTPUT_DIR")",
    "summary": "$(json_escape "$SUMMARY_OUTPUT_PATH")",
    "traceability": "$(json_escape "$TRACEABILITY_OUTPUT_DIR")",
  ...
  "safety": {
    ...
    "packages_existing_artifacts_only": true,
...
for path_value in "$WORKDIR" "$RESULTS_ROOT" "$INDEX_PATH" "$FIGURE_OUTPUT_DIR" "$SUMMARY_OUTPUT_PATH" "$TRACEABILITY_OUTPUT_DIR" "$MANIFEST_PATH" "$SCHEDULER_SCRIPT_PATH" "$WRAPPER_PATH" "$STDOUT_PATH" "$STDERR_PATH" "$AUDIT_PATH" "$LOG_DIR"; do
  if [[ -n "$path_value" ]]; then
    validate_posix_path "path" "$path_value"
  fi
done
mkdir -p "$WORKDIR/results" "$LOG_DIR"
RUN_LOG="$LOG_DIR/report_generator.log"
exec > >(tee -a "$RUN_LOG") 2>&1

write_run_manifest "$([[ "$DRY_RUN" == "true" ]] && printf 'dry_run' || printf 'execute')"
if [[ "$DRY_RUN" == "true" ]]; then
  log "Dry-run complete; report generator manifest written without running packaging scripts."
  exit 0
fi
