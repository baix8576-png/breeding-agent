...
## 项目定位
GeneAgent 是面向动植物遗传育种研究者的智能助手，支持用户通过自然语言完成群体遗传学与数量遗传学分析编排，支持在本地 Linux 集群、macOS、Windows（含 WSL2）、HPC集群上自动生成 Bash 脚本、提交 SLURM 任务、轮询作业状态、整理结果与报错信息。
...
- 部署位置：Linux系统、HPC集群、macOS、Windows + WSL2等
...
## Roadmap
- `v1`：打通 `bioinformatics / system / knowledge` 三类请求分流，完成 `qc / pca / grm / genomic_prediction` 四条主链，提供 CLI/API、SLURM 主线、基础知识库、输入校验、dry-run/submit/poll、审计与记忆回写、熔断机制。
- `v1.5`：补强 PBS 兼容、报告生成、更多工具 manifest、增强诊断与更完整的本地知识检索。
- `v2`：扩展记忆系统、自动结果解释、Web UI、插件化工具生态，以及更多育种与多组学场景模板。
